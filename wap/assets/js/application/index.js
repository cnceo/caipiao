            ...
            success: function(results) {
                ...
                if(results.status == "200" && results.data != null) {
                	...
                	var html = '<div id="pop_notice" class="notice-index" style="max-height:350px;overflow-y:scroll;"><dl>'
			        			+'<dt id="pop_title">'+a[i].title+'</dt>'
			        			//+'<dd id="pop_time" class="time">'+a[i].addTime+'</dd>'
			        			//+'<dd id="pop_content" class="cont">'+a[i].content+'</dd>'
			        		  +'</dl></div>';}
                	layer.open({
    				    content: html,
    				    btn: ['关闭'],
    	...
    	$.ajax({
            ...
            success: function(results) {
                ...
                if(results.status == "200" ) {
                	...
                	var html = '<div id="pop_notice" class="notice-index" style="max-height:350px;overflow-y:scroll;"><dl>'
			        			+'<dt id="pop_title">您有未读消息</dt>'
			        			+'<dd id="pop_time" class="time"></dd>'
			        			+'<dd id="pop_content" class="cont"></dd>'
			        		  +'</dl></div>';
                	layer.open({
    				    content: html,
    				    btn: ['去查看'],
