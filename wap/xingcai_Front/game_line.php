...
.bett-attr {
    background: url(https://www.cp3366.com/wap/statics/images/betting/top_zhong_sanjiao.png) no-repeat right center;
    width: 12.5px;
    height: 8.5px;
...
<div class="lotteryBody">
    ...
    if($this->type == 83 || $this->type == 34|| $this->type == 77){
        ...
        echo '<div id="nfdprize-text" class="m-lott-methodBox" style="height: 70px;padding:  10px;">';
    }
    ?>

      <span class="nfdprize_text" id="m-lott-listContent"><span id="wfbh">玩法选择</span><i class="bett-attr"></i></span>
      <?php
      if($this->type == 83 || $this->type == 34|| $this->type == 77){
             ...
             <div id="submenu" >
							...
							if ($playeds)
							    foreach ($playeds as $played) {
							        ...
							        if ($played['enable']) {
							?>
								<a data_id="<?= $played['id'] ?>" href="#" tourl="/index.php/index/played_new/<?= $this->type ?>/<?= $played['id'] ?>" <?= ($played['id'] == $this->played) ? ' class' : '' ?> style="white-space: nowrap;overflow: hidden;text-overflow: ellipsis;"><?= $played['name'] ?></a>
								<?php
							        }
							    }
							?>
