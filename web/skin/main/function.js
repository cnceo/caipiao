...
function showBetInfo(id) {
    $.get('/index.php/record/betInfo/' + id,
    function(data) {
        $(data).dialog({
            title: '投注信息',
            width: 500,
...
function newcode(code_arr) {
    var _html = '';
    if (game.type == 20 || game.type == 65 || game.type == 66 || game.type == 85) { //pk10开奖动画
        $.each(code_arr,
...
function getQiHao() {
    $.getJSON('/index.php/index/getQiHao/' + game.type,
    function(data) {
...
function setGameZhuiHao(data) {
    ...
    $.get('/index.php/index/zhuiHaoModal',
    function(html) {
        $(html).dialog({
...
function gameAddCode(code) {
    wait();
    var actionNo = $.parseJSON($.ajax('/index.php/game/checkBuy', {
        async: false
    }).responseText);
    destroyWait();
    if (actionNo) {
...
function gamePostCode() {
    ...
    wait();
    var actionNo = $.parseJSON($.ajax('/index.php/game/getNo/' + game.type, {
        async: false
    }).responseText);
    destroyWait();
    if (!actionNo) {
