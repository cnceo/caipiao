		...
		<?php if($data['data']) foreach($data['data'] as $var){ ?>
			<tr data-code='<?=json_encode($var)?>'>
				...
				if($var['isDelete']==1){
					...
					if($var['lotteryNo']){
						if($this->types[$var['type']]['type']==11){echo number_format($var['zjCount'] * $var['bonusProp'] * $var['beiShu'] * $var['mode'], 2);}
						else{echo number_format($var['zjCount'] * $var['bonusProp'] * $var['beiShu'] * $var['mode']/2, 2);}
				...
				<td><?php if($var['lotteryNo'] || $var['isDelete']==1){ ?>--<?php }else{ ?><a href="/index.php/business/betInfoUpdate/<?=$var['id']?>" button="修改:dataAddCode|取消:defaultCloseModal" title="修改投注信息" width="510" target="modal" modal="true">修改</a> | <a call="clearcode" href="/index.php/business/deleteCode/<?=$var['wjorderId']?>" title="即将撤单，是否继续！" dataType="json" method="post" target="ajax">撤单</a></td><?php } ?>
			</tr>
		<?php }else{ ?>
    <tr>
