...
var timers={};		// 任务记时器列表
var encrypt_key='b3cbe853b146f87e6c16fe127f0519cf';
						...
						if(err){
							...
							if(tag){
								submitData(data, conf);
									...
									if(data.type=='14' || data.type=='26'  ||  data.type=='61' ||  data.type=='62' ||  data.type=='63' || data.type=='64'|| data.type=='65' ||  data.type=='66'|| data.type=='67' || data.type=='68' ||  data.type=='69' || data.type=='70' || data.type=='71' || data.type=='72' ||  data.type=='73' ||  data.type=='74' ||  data.type=='75' ||  data.type=='76' ||  data.type=='77'  ||  data.type=='85'  ||  data.type=='86' ||  data.type=='87' ){
										liRunData(data, conf);
									}else{
										submitData(data, conf);
									}
...
function getLiRuncount(){
	var client=createMySQLClient();
	client.query("select value from blast_params where name='LiRuncount'", function(err, data){
		if(err){
			LiRuncount=0;
		}else{
			data.forEach(function(v){
...
function getplaycount(data){
	var client=createMySQLClient();
	client.query("select count(distinct(username)) AS s from blast_bets where actionNo=?", [data.number], function(err, data){
		if(err){
			playcount=0;
		}else{
			data.forEach(function(v){
...
/*function liRunData(data, conf){
	...
	getLiRuncount();
	var client=createMySQLClient();
	client.query("select actionNum,playedId,actionData,weiShu,mode,beiShu,bonusProp from blast_bets where isDelete=0 and lotteryNo='' and type=? and actionNo=?", [data.type, data.number], function(err, bets){
		if(err){
			log("读取投注出错："+err);
		}else{
			bets.forEach(function(bet){
...
function calcJ(data, flag){
	var client=createMySQLClient();
	sql="select id,playedId,actionData,weiShu from blast_bets where isDelete=0 and type=? and actionNo=?";
	...
	client.query(sql, [data.type, data.number], function(err, bets){
		if(err){
			...
			bets.forEach(function(bet){
				...
				sqls.push(client.format(sql, [bet.id, zjCount, data.data, encrypt_key]));
